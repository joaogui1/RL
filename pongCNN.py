...
from karpathy import preproCNN, discount_rewards


model = Sequential()
model.add(Conv2D(64, kernel_size=(5, 5), activation='relu', padding = 'same', input_shape=(1, 80, 80)))
...
while(True):
    ...
    cur_input = preproCNN(observation)
    x = cur_input - prev_input if prev_input is not None else np.zeros((80, 80))
    prev_input = cur_input

    # forward the policy network and sample action according to the proba distribution
    proba = model.predict(np.expand_dims(x, axis=2).T)
